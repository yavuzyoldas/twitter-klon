...
return dispatch =>  {
    ...
    firebase.database().ref('/tweets').push({email,tweet,date}).then(() => {
      history.push('/');
...
return dispatch => {
  ...
  firebase.database().ref(REF_DATABASE + '/' + uid).set({ tweet , date ,email}).then(() => {
    history.push('/');
...
return dispatch => {
  firebase.database().ref(REF_DATABASE).orderByChild('email').equalTo(email).on('value',snapshot => {
    const result = _.map(snapshot.val(),(val , uid ) =>{
      return {...val, uid};
    })
